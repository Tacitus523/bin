...
def main():
    ...
    ap.add_argument("-e", type=str, dest="env_energy_file", action="store", required=True, help="File with energy data in environment [H]", metavar="environment energy file") # in Hartree
    ap.add_argument("-c", type=str, dest="env_charge_file", action="store", required=True, help="File with charges in environment [H]", metavar="env charge file") # in Hartree
    ap.add_argument("-p", type=str, dest="env_esp_file", action="store", required=True, help="File with electrostatic potentials from environment [H]", metavar="ESP file") # in Volt
    ...
    true_e_elec = (env_energies - vacuum_energies) *H_to_kcal_mol
    calc_e_elec = np.sum(env_charges*env_esps, axis=-1) *H_to_kcal_mol
    rmse = mean_squared_error(true_e_elec, calc_e_elec, squared=False)
    r2 = r2_score(true_e_elec, calc_e_elec)

    data["True E_elec"] = true_e_elec 
    data["Calculated E_elec"] = calc_e_elec
