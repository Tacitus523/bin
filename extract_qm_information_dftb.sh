...
TMP_GRADIENTS_FILE="gradients_tmp.xyz"
...
# Write a gradients file in xyz format based on the .xyz file, converts forces to gradients
write_gradients_xyz() {
	local detail_file=$1
	local xyz_file=$2
	local num_atoms=$3
	local gradients_file=$4

	local tmp_geom_file="_tmp_geom.xyz"
	local tmp_gradients_file="_tmp_gradients.txt"

	if [ $detail_file == $tmp_geom_file ] || [ $detail_file == $tmp_gradients_file ]
	then
	...
	grep -A $(($num_atoms+0)) -m 1 "Total Forces" $detail_file | awk 'FNR>1{printf "%+4.9f %+4.9f %+4.9f\n", -$2, -$3, -$4}' > $tmp_gradients_file
	tail -n $num_atoms $xyz_file > $tmp_geom_file

	head -n 2 $xyz_file > $gradients_file
	awk 'NR==FNR{a[NR]=$1;next}{print a[FNR], $1, $2, $3}' $tmp_geom_file $tmp_gradients_file >> $gradients_file

	rm $tmp_geom_file
	rm $tmp_gradients_file
}
...
do
	...
	write_gradients_xyz $folder/$DETAIL_FILE $folder/$xyz_file $num_atoms $folder/$TMP_GRADIENTS_FILE
