...
def extract_data(
    ...
    result["forces"] = np.array(ref_forces) #* H_B_to_eV_A # Convert Hartree/Bohr to eV/Å
    if ref_charges:
...
def plot_data(
    ...
    # Use offset notation if values are larger than 1000
    abs_max = max(abs(df[x_label].max()), abs(df[y_label].max()))
    
    if abs_max > 1000:
        # Lower the offset threshold to make offset notation more likely
        plt.rcParams["axes.formatter.offset_threshold"] = 3
        
        # Use automatic offset formatting
        formatter_x = ScalarFormatter(useOffset=True, useMathText=True)
        formatter_x.set_useOffset(True)
        formatter_y = ScalarFormatter(useOffset=True, useMathText=True)
        formatter_y.set_useOffset(True)
